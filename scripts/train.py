...
from birefnet.utils import (
    Logger,
    AverageMeter,
    set_seed,
    check_state_dict,
    init_wandb,
    get_lr_warm_up_scheduler,
)
...
def prepare_dataloader(
    ...
    if to_be_distributed:
        return torch.utils.data.DataLoader(
            ...
            if is_train and config.dynamic_size != (0, 0)
            else None,
        )
    else:
        return torch.utils.data.DataLoader(
...
def get_scores(list_gt: List[str], list_pred: List[str]):
    ...
    scores = {
        metric: value["curve"].mean().round(3)
        if metric in ["E", "F", "BIoU"]
        else int(hce.round())
        if metric == "HCE"
        else value.round(3)
    ...
    return {
        ...
        if metric in config.display_eval_metrics
    }
...
def init_data_loaders(to_be_distributed):
    ...
    validation_set = os.path.join(
        config.data_root_dir, config.task, args.validation_set
    )
    ...
    print(
        len(train_loader),
        "batches of train dataloader {} have been created.".format(training_set),
    )
    print(
        len(validation_loader),
        "batches of validation dataloader {} have been created.".format(validation_set),
    )
...
def init_models_optimizers(epochs, to_be_distributed):
    ...
    wu_epochs = 10
    warmup_scheduler = get_lr_warm_up_scheduler(
        config.lr_warm_up_type, wu_epochs, 1e-20, 1.0, optimizer
    )

    # Main scheduler after warmup
    main_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=config.lr_decay_epochs, gamma=config.lr_decay_rate
    )
...
class Trainer:
    ...
    def iteration_over_batches_validation(
        self, epoch, info_progress=None, step_idx=None, training_result=None
    ):
        ...
        loss_components_dict = {
            loss_name: loss_accumulated_value / len(self.validation_loader)
            for loss_name, loss_accumulated_value in loss_components_dict.items()
        }
        validation_metrics_dict = {
            metric: score / len(self.validation_loader)
            for metric, score in validation_metrics_dict.items()
        }
        ...
        loss_components_dict = {
            loss_name: self.average_between_devices(loss_accumulated_value)
            for loss_name, loss_accumulated_value in loss_components_dict.items()
        }
        validation_metrics_dict = {
            metric: self.average_between_devices(score)
            for metric, score in validation_metrics_dict.items()
        }
    ...
    def iteration_over_batches_train(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            step_idx = (
                batch_idx + len(self.train_loader) * (epoch - config.start_epoch) + 1
            )
            ...
            if batch_idx % config.log_each_steps == 0:
                ...
                loss_general_value = self.average_between_devices(
                    self.loss_dict_train[self._get_loss_key(epoch)]
                )
                self.loss_components_train = {
                    loss_name: self.average_between_devices(loss_value)
                    for loss_name, loss_value in self.loss_components_train.items()
                }
                accelerator.wait_for_everyone()
                if accelerator.is_main_process:
                    info_loss = f"Training Losses {loss_general_value}"
                    logger.info(" ".join((info_progress, info_loss)))
                self.iteration_over_batches_validation(
                    epoch, info_progress, step_idx, loss_general_value
                )
    ...
    def epoch_final_logs(self, epoch, log_losses, log_task="Training"):
        ...
        if accelerator.is_main_process:
            logger.info(
                f"@==Final== Epoch[{epoch}/{args.epochs}]  Average {log_task} Loss: {avg_loss}"
            )
        ...
        if not validation:
            ...
            if args.use_accelerate:
                ...
                accelerator.clip_grad_norm_(
                    self.model.parameters(), config.gradient_clipping_norm
                )
            else:
                loss.backward()
...
def main():
    ...
    for epoch in range(epoch_st, args.epochs + 1):
        ...
        # DDP
        if (
            epoch >= trainer.save_last_epochs_start
            and (args.epochs - epoch) % args.save_each_epochs == 0
        ):
            if save_to_cpu:
            ...
            # default behavior
                ...
                if args.use_accelerate:
                    if mixed_precision == "fp16":
                        state_dict = {
                            k: v.half() for k, v in trainer.model.state_dict().items()
                        }
                else:
                    state_dict = (
            ...
            torch.save(
                state_dict, os.path.join(args.ckpt_dir, "epoch_{}.pth".format(epoch))
            )
    if to_be_distributed:
